...
이 프로젝트는 사용자들이 책을 더 효율적으로 접하고 이해할 수 있도록 도와주는 AI 기반 독서 요약 플랫폼입니다.
...
> Java 17과 Spring Boot 3.3.9를 선택한 이유는 강력한 타입 안정성, 풍부한 라이브러리 생태계, 그리고 확장 가능한 엔터프라이즈급 애플리케이션 개발에 적합하기 때문입니다. 특히 Spring
...
> MySQL을 주 데이터베이스로, Redis를 캐싱 솔루션으로 선택했습니다. MySQL은 안정성과 신뢰성이 검증된 관계형 데이터베이스이며, 트랜잭션 처리와 복잡한 쿼리를 지원합니다. JPA는 객체-관계 매핑을
> 간소화하고 데이터베이스 독립적인 코드 작성을 가능하게 합니다.
...
> 지속적 통합 및 배포를 위해 GitHub Actions를 활용하고, 테스트 자동화를 위해 JUnit과 Mockito를 사용합니다. 이를 통해 코드 품질을 유지하고 배포 프로세스를 간소화합니다.
...
> 비동기 처리와 시스템 성능 향상을 위해 RabbitMQ와 Spring Async를 도입했습니다. 이를 통해 사용자 요청에 대한 응답 시간을 최소화하고, 시스템 부하를 분산시킵니다.
...
![TF-IDF](https://img.shields.io/badge/TF--IDF-검색_관련성_점수-00ADD8?style=flat-square&logo=go)
![감정 분석 모델](https://img.shields.io/badge/감정_분석-텍스트_감정_추출-FF5733?style=flat-square&logo=tensorflow)
![페이지랭크 변형](https://img.shields.io/badge/페이지랭크_변형-콘텐츠_인기도_측정-0088CC?style=flat-square&logo=pagekit)
![LRU 캐싱](https://img.shields.io/badge/LRU_캐싱-검색어_캐싱-06B6D4?style=flat-square&logo=tailwindcss)
...
- **랭킹 시스템**: `ranking_user_score`, `ranking_user_activity`, `ranking_leaderboard`, `ranking_badge`,
  `ranking_achievement`

#### 참고사항

- 테스트 데이터 로드 시 외래 키 제약 조건으로 인한 오류가 발생할 수 있습니다. 이를 방지하기 위해 `data_utf8.sql` 파일에서는 `SET FOREIGN_KEY_CHECKS = 0;`로 제약 조건을
  비활성화하고, 데이터 삽입 후 `SET FOREIGN_KEY_CHECKS = 1;`로 다시 활성화합니다.
...
-- 데이터베이스 초기화
DROP
DATABASE IF EXISTS stoblyx_sandbox_db;
CREATE
DATABASE stoblyx_sandbox_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE
stoblyx_sandbox_db;
...
2. **외래 키 제약 조건 오류** - 참조하는 데이터가 없는 경우 발생합니다.
   ...
   해결 방법: `data.sql` 파일 시작 부분에 `SET FOREIGN_KEY_CHECKS = 0;`를 추가하고 끝 부분에 `SET FOREIGN_KEY_CHECKS = 1;`을 추가하여 외래 키 제약 조건을
   임시로 비활성화합니다.
...
데이터베이스는 사용자, 콘텐츠, 상호작용, 랭킹 시스템 등 다양한 도메인으로.구성된 총 29개의 테이블을 포함합니다.
...
프로젝트의 테스트 전략은 효율성과 실용성을 중시하여 기존 테스트 피라미드 개념을 컨트롤러 테스트와 k6 테스트의 조합으로 재해석하였습니다.
...
스토블릭스는 알라딘 API에서 제공하지 않는 도서 정보를 자동으로 보강하는 **BookEnrichmentService**를 구현했습니다. 이 서비스는 다음과 같은 필드를 자동으로 계산하여 채워 줍니다.
...
이 보강 알고리즘은 API 호출 결과를 자동으로 처리하여 더 풍부한 사용자 경험을 제공합니다.
...
신간 도서 API 호출 결과에는 도서의 출간일이 최근인 순서로 정렬되어 있으며, 각 도서의 상세 정보와 함께 스토블릭스의 데이터 보강 알고리즘을 통해 계산된 정보가 추가되어 있습니다. 이를 통해 사용자는 신간 도서의 예상 읽기 시간과 초기 인기도를 확인할 수 있습니다.
...
검색 결과에는 키워드 관련성이 높은 순서로 도서들이 정렬되어 있으며, 각 도서마다 기본 정보와 함께 스토블릭스가 보강한 정보가 제공됩니다. 특히 프로그래밍 서적의 경우, 장르 특성상 평균 읽기 시간이 더 길게 계산되는 것을 확인할 수 있습니다.
...
ISBN 검색은 가장 정확한 도서 정보를 제공하며, 스토블릭스의 보강 알고리즘을 통해 이 장편 소설의 평균 읽기 시간이 300분 이상으로 계산되었고, 2011년 출간된 오래된 도서임에도 높은 인기도 점수를 유지하고 있는 것을 확인할 수 있습니다.
...
- 이미지 캐싱을 통한 응답 속도 향상
- 주기적 데이터 동기화로 최신 도서 정보 유지
- 장르 정규화를 통한 일관된 데이터 품질 유지
- 데이터 보강 알고리즘의 자동 적용

### 향후 개선사항

- 사용자 독서 이력 기반의 읽기 시간 예측 정확도 향상
- 협업 필터링을 활용한 개인화된 인기도 점수 부여
- 대용량 데이터 처리를 위한 배치 처리 최적화
- 머신러닝 모델을 활용한 더 정확한 평점 예측
- 사용자 피드백을 통한 알고리즘 지속적 개선
