...
public class SystemSettingInitializer {
    ...
    public CommandLineRunner initSystemSettings(SystemSettingRepository repository) {
        return args -> {
            // 이미 초기화되었는지 확인 - API 키 설정을 확인해서 구체적인 검사 수행
            if (repository.existsByKey("api.key.pexels")) {
                log.info("시스템 설정이 이미 초기화되어 있습니다. 초기화 작업을 건너뜁니다.");
                return;
            }

            log.info("시스템 설정을 초기화합니다...");
            initSettingsWithoutTransaction(repository);
            log.info("시스템 설정 초기화가 완료되었습니다.");
        };
    }
    
    /**
     * 트랜잭션 없이 시스템 설정을 초기화합니다.
     * 
     * @param repository 시스템 설정 리포지토리
     */
    private void initSettingsWithoutTransaction(SystemSettingRepository repository) {
        // 기본 시스템 설정 목록 생성
        ...
        // 각 설정을 개별적으로 저장하여 배치 처리 방지
        for (SystemSetting setting : defaultSettings) {
            try {
                if (!repository.existsByKey(setting.getKey())) {
                    repository.save(setting);
                    log.debug("시스템 설정 저장: {}", setting.getKey());
                }
            } catch (Exception e) {
                log.error("시스템 설정 저장 중 오류 발생: {}", setting.getKey(), e);
            }
        }
    }
} 