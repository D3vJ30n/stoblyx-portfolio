...
public interface PostRepository extends JpaRepository<Post, Long> {
    @Query("SELECT p FROM Post p WHERE p.author.id = :userId AND p.isDeleted = false")
    Page<Post> findByAuthorId(@Param("userId") Long userId, Pageable pageable);

    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM Post p WHERE p.id = :id AND p.author.id = :userId")
